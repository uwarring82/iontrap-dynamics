function [P, nph, meansz, meansx, meansy, varsz, varsx, varsy, 
  eta0] = sb_evolution _modes _Bloch (N, center, nc, nph, Omega, 
   omegaz, axial, theta, phi, tmin, tmax, tstep)
%
% INPUT : 
   %       N = number of ions
%       center = 
 position of the spin in the chain (e.g. center = 1, spin at ion 1)
%       nc = 
 cut - off of maximum number of phonons in the calculation
%
%       nph = 
 vector with the mean phonon numbers for each vibrational
%       mode, e.g., nph = [0.1, 0.2, 0.3] -> 
  mean values 0.1, 0.2, 0.3 for
%       axial vibrational modes n = 1, 2, 3, respectively
%
%       T = 
 temperature (e.g., T = 100 with axial frequency 1, 
   means < n > = 100)
...
%       theta, phi = angles in the Bloch sphere of the intial state :
    %           \\ psi = 
  cos (theta/2) | up > + sin (theta/2) e^(-i \\ phi) | down >
    %       tmin, tmax, tstep :  
 minimum evolution time, maximum ev. time and time step
...
%       nph = 
 number of phonons : nth column corresponds to nth vibrational mode
%                                nth file is the nth time step
%       meansz/x/y = time averaged value of < sigma^z/x/y >
   %       varsz/x/y = 
  variance of the flucutations in time of < sigma^z/x/y >
   %                                   
...
Id = eye (length (On_cell {1}));
    ...
    pol = 0;
    Hn = 0;
    eta0 = eta_calculator (25, 279.65/sqrt (2), axial);
    for n = 1 : N
        pol = 
  pol + i*eta0*
    center_wf (n)*(1/sqrt (ener (n)/ener (1)))*(Oa_cell {n} + 
      Oa_cell {n}');
...
% prepare the state in a pure initial spin state with angles (theta, 
  phi)
% on the Bloch sphere
rho0 = rho0*(Id + cos (theta)*Osz + sin (theta)*cos (phi)*Osx + 
      sin (theta)*sin (phi)*Osy)/2;
...
%
function [Oa_cell, On_cell, Oid_cell, Osz, Osm, Osp] = 
 Operators (nc, Nsites)
%
n = 1;
j = n/2;
...
X0 = initial (n);
options = 
  optimset (' GradObj', ' on', ' Hessian', ' on', ' TolX', 
    10^-16, ' TolFun', 10^-16, ' Display', ' off');
[Xmin fval exitflag output grad Hessian] = 
  fminunc (@mypotential, X0, options, 4, n);
...
% we give an estimation of the equilibrium position of the ions as \
initial
