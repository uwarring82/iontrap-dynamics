function [P,nph,meansz,meansx,meansy,varsz,varsx,varsy,nphF] = sb_evolution_modes_Bloch(N,center,nc,nph,Omega,omegaz,axial,theta,phi,tmin,tmax,tstep)
%
% INPUT:
%       N = number of ions
...
%       nphF(c,n,nF) is an array that contains the diagonal elements of the phonon density matrix
%           c is the step (so that nphF is calculated at time P(1,c))
%           n is the vibrational mode
%           nF is the Fock state (nF=1 is Fock state 0, nF=2 is Fock sate 1, ...)
%
...
ener = ener*axial;
% define operators
[Oa_cell,On_cell,Oid_cell,Osz,Osm,Osp,OnF_cell] = Operators(nc,N);
Osx = Osp + Osm;
Osy = -i*(Osp - Osm);
...
On_cell{n} = VV'*On_cell{n}*VV;
for nF = 1:1+nc
OnF_cell{n,nF} = VV'*OnF_cell{n,nF}*VV;
end
end
...
for nF = 1:nc+1
nphF(count,n,nF) = real(trace(rho*OnF_cell{n,nF}));
end
end
count = count + 1;
%disp(t)
...
%
%
function [Oa_cell,On_cell,Oid_cell,Osz,Osm,Osp,OnF_cell] = Operators(nc,Nsites)
%
nnF = cell(1+nc);
n = 1;
j = n/2;
...
for nF = 1:nc+1
nnF{nF} = diag([zeros(1,nF-1),1,zeros(1,nc-nF+1)]);
end
%nn =  sparse(diag([0:nc]));
nn = a'*a;
is = speye(n+1);
...
OnF_cell{j,nF} = 1;
for jj = 1:j-1
OnF_cell{j,nF} = kron(ip,OnF_cell{j,nF});
end
OnF_cell{j,nF} = kron(nnF{nF},OnF_cell{j,nF});
for jj = j+1:Nsites
OnF_cell{j,nF} = kron(ip,OnF_cell{j,nF});
end
OnF_cell{j,nF} = kron(is,OnF_cell{j,nF});
end
end
%
