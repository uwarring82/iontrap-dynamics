function [meanE,DE,enerf,szdiag,sxdiag,nphdiag_cell,sz,sx,sy,IPR,IPR0,ratio_IPR,IPR_av,etaout] = ergodic(N,nc,n_val,Omega,omegaz,axial,theta,phi)
%
...
% IPR0 = Inverse Participation Ratio at zero spin-boson coupling
% ratio_IPR = IPR/IPR0 = Effective number of states per initial states in
...
%IPR_av2 = IPR_av2 + 2*D0(j,j)*sqrt(r2);
%     end
%IPR_j(j) = 1/sum(rho_dj.^2);
end
%IPR_av2 = 1/IPR_av2;
%
...
%
% calculate 300 first eigenstates
Neigens = min(300,length(H)-1);
%
%[VVf,enerf0] = eigs(H,size(H,1));
[VVf,enerf0] = eig(H);
%
rho_d = diag(VVf(:,:)'*rho0*VVf(:,:));
IPR0 = 1/sum(rho_d.^2);
