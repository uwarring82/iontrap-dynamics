function [meanE,DE,enerf,szdiag,sxdiag,nphdiag_cell,sz,sx,sy,IPR,IPR_av,Deff_mat] = ergodic_inf_cutoff(N,nc,n_val,Omega,omegaz,axial,theta,phi)
%
...
%Neigens = min(Neigen,length(H))
%
%[VVf,enerf] = eigs(H,Neigens,meanE-DE);
[VVf,enerf] = eig(H);
%
%
enerf = diag(enerf);
...
%IPR_av2 = 1/sum(sqrt((sum(abs(VVf(:,:)'*V0(:,:)).^4)))*D0);
%IPR_av2 = IPR_av2^2;
%
Deff_mat = (1./sum(abs(VVf(:,:)'*V0(:,:)).^4));
%rhodiag = diag(D0);
%
