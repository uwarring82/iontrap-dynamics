...
| **WI-3** | F3 | **new `channels.py`** + `sequences.solve` | typed dissipators `AmplitudeDamping(mode, rate)`, `Heating(mode, n_bar_bath, rate)`, `Dephasing(mode, rate)`, `Depolarising(…)` → QuTiP collapse ops **or** Kraus; `solve(…, channels=[…])` routed to the `mesolve` path; **segment-bound** application over a pulse sequence | `sequences.solve` (`mesolve` path), `operators.py` | analytic ⟨n̂⟩ decay (damping); coherence decay (dephasing); n̄→n̄_bath steady state (heating); CPTP/trace invariants; **R8 test: two non-commuting channels are order-dependent** | **§24** (freeze) | **P0 (pivotal)** |
...
| `MCC` | WI-3 motional CPTP channels + `solve(channels=…)` | `- **Dispatch MCC — channels: typed motional CPTP channels exposed in solve().**` | **WI-3a landed 2026-06-02** (3 dissipators + `solve(channels=…)`; oracles green, 1014 tests); WI-3b (windowed + R8) open |
