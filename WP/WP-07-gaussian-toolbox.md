...
Version 0.1 · Drafted 2026-07-14 · Ratified and sealed 2026-07-14 · Status: **Ratified** (GT1 landed + §27 sealed at `CONVENTION_VERSION` 0.6, 2026-07-14; GT2 purity/entropy + GT4 log-negativity/separability + GT5 effective-temperature landed 2026-07-15; **GT3a generic congruence + GT3b ion `S`-adapter landed 2026-07-15** (GT3b consumer side; producer `IonModeBasis` emitter = independent `iontrap-structure` work); **GT6 locally-symmetric `E_F` landed 2026-07-15 — WP-07 code-complete**)
...
| **GT3a** | `gaussian.py` | generic `congruence(S, V)` = `S V Sᵀ` with a **scale-aware** `SΩSᵀ=Ω` guard; validates real/finite/square `2N×2N` `S` matching `V`, and both input + output as physical covariances; preserves the Williamson spectrum `ν_i` (hence purity/entropy/physicality) — **consumes GT1's `symplectic_form` + `symplectic_eigenvalues`** | — (application-agnostic; **no** ion machinery) | identity; single-mode squeeze `diag(e^r,e^{-r})` (vacuum → `ν=1`) + rotation; two-mode beamsplitter `ν`-preserved; `S⁻¹` round-trip; physicality preserved; nonsymplectic + malformed `S` (shape/complex/NaN) rejected | `GT3a` | **landed 2026-07-15** |
| **GT3b** | `ion_modes.py` (adapter; **not** `gaussian.py`) + producer payload (separate, `iontrap-structure`) | **ion-specific `S`** (normal→local) built in a new `iontrap-dynamics` adapter from a versioned `IonModeBasis` payload (complete `3N` basis + `ω` + local reference frequencies; mass **cancels**): `S = diag(X, P)`, `X=B√(ω_loc/ω)`, `P=B√(ω/ω_loc)`, per-mode ordering, feeds GT3a `congruence`; consumer-owned schema, no shared runtime class | ratified handshake card `TC-gt3b-ion-symplectic-adapter.md` (`1a88145`); consumes GT3a `congruence` + GT4 `log_negativity` | canonicality (equal/mixed mass); **coupled ground state == independent QuTiP diagonalization** (machine precision); COM/stretch `3^{±1/4}`; passive-limit separability; `ω_loc` gauge-invariance of `E_N`; payload contract (schema/frame/Gram) | `GT3b` | **adapter landed 2026-07-15** (consumer side; producer emitter = independent `iontrap-structure` work) |
...
| **GT5** | `gaussian.py` | `effective_temperature(nbar, omega_loc)` = `ℏω_loc/(k_B ln(1+1/n̄))` [K] with first-moment-aware `n̄` (from `mean_occupation`); explicit angular `ω_loc` (rad/s), SI `ℏ`/`k_B` (module-local CODATA); `T_eff(0)=0`; rejects `n̄<0`/NaN and `ω_loc≤0`/NaN; neutral symbol | `qutip.thermal_dm` + analytic Bose (round-trip oracle) | thermal round-trip `n̄→T_eff→n̄` exact; a **squeezed and/or displaced** marginal gives `T_eff>0` (thermal-core `(ν−1)/2` would give 0); energy-equivalent | `GT5` | **landed 2026-07-15** |
| **GT6** | `gaussian.py` | locally-symmetric Gaussian `E_F` = `g((ν̃₋+1/ν̃₋)/2)` for `ν̃₋<1` else 0 (Giedke et al. 2003), reusing GT1's PT-spectrum + GT2's `g`; **local-symmetry precondition on the invariant `det A = det B`** (relative, admits locally-equivalent states, scale-robust); faithful (no clamp — weak entanglement gives small positive `E_F`) | — | pure TMSV → entropy of entanglement (independent oracle); mixed symmetric → analytic `ν̃₋=ν₀e^{-2r}`; brute-force EoF-minimization cross-check; separable → 0. *Generic `E_F` deferred.* | `GT6` | **landed 2026-07-15** |

## 5. Sequencing and gates *(Coastline gate)*

**Order:** ratify → mint `GT`, stage §27 seal → **GT1** (§27 seal + the convention primitives: covariance, symplectic form, physicality, symplectic spectrum, PT — conventions-before-code, nothing consumes `V` before the seal) → GT2 (purity/entropy) → GT4 (log-negativity, the headline consumer functional) → GT5 (`T_eff`) → GT3a (generic congruence) + GT3b (ion `S`-adapter — the two-ion Phase B dependency) → GT6 (optional `E_F`). GT2/GT4/GT5 are pure `gaussian.py` linear algebra over GT1's spectrum + PT and can parallelise once GT1 lands. **GT1 landed and §27 sealed 2026-07-14** (conventions test green; `CONVENTION_VERSION` 0.5 → 0.6); **GT2 (purity/entropy) landed 2026-07-15**; **GT4 (log-negativity + `is_separable`) landed 2026-07-15** (full-sum `E_N`, exact Fock cross-check, `1×N`-scoped strict certificate); **GT5 (`effective_temperature`) landed 2026-07-15** (`ℏω/(k_B ln(1+1/n̄))`, exact thermal round-trip, first-moment-aware); **GT3a (generic `congruence`) landed 2026-07-15** (scale-aware `SΩSᵀ=Ω` guard, ν-preserving, input+output physicality-validated — application-agnostic, no ion machinery); **GT3b (ion `S`-adapter) landed 2026-07-15** (`ion_modes.py` consumer adapter against the ratified `IonModeBasis` handshake; coupled ground state matches an independent QuTiP diagonalization to machine precision; producer emitter is independent `iontrap-structure` work); **GT6 (`entanglement_of_formation`) landed 2026-07-15** (Giedke-et-al. `g((ν̃₋+1/ν̃₋)/2)`; `det A = det B` local-symmetry invariant; faithful — no weak-entanglement clamp; validated against an independent brute-force EoF minimization). **WP-07 is code-complete: GT1–GT6 all landed.**
...
| `GT3a` | WI-GT3 | `- **Dispatch GT3a — generic symplectic congruence V ↦ S V Sᵀ.**` | **landed 2026-07-15** |
| `GT3b` | WI-GT3 | `- **Dispatch GT3b — ion S-adapter (normal→local).**` | **adapter landed 2026-07-15** (`ion_modes.py`; producer emitter = independent `iontrap-structure` work) |
| `GT4` | WI-GT4 | `- **Dispatch GT4 — arbitrary-cut Gaussian log-negativity + cut semantics.**` | **landed 2026-07-15** |
| `GT5` | WI-GT5 | `- **Dispatch GT5 — effective temperature (neutral T_eff).**` | **landed 2026-07-15** |
| `GT6` | WI-GT6 | `- **Dispatch GT6 — locally-symmetric Gaussian E_F (optional).**` | **landed 2026-07-15** (`entanglement_of_formation`; `det A = det B` invariant; faithful; brute-force EoF-verified) |
