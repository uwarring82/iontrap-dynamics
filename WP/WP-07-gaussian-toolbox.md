...
| **GT3b** | ion `S`-adapter (separate, near `iontrap-structure`) | **ion-specific `S`** (normal→local from a complete basis + cross-mode orthogonality + masses + local frequencies) — a **separate scoped cross-repository decision**, kept **out** of `gaussian.py` | `ModeConfig.eigenvector_per_ion` (necessary-not-sufficient) | **squeezed** normal-mode product → correct ion-cut `E_N`; non-orthogonal (COM vs stretch) case; consumes GT3a's `congruence` | `GT3b` | open |
...
| `GT3b` | WI-GT3 | `- **Dispatch GT3b — ion S-adapter (normal→local).**` | minted, open (separate cross-repo decision) |
