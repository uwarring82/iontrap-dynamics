...
- **Dispatch GT1 — the multimode Gaussian convention primitives (§27).**
  ...
  similar-Hermitian form `eigvalsh(i V^{1/2} Ω V^{1/2})` (no silent `.real` discard). A `V`
  outside the certified numerical range (`cond ≳ 1e12` — physical or not; an analytic pure
  squeeze `diag(1e6, 1e-6)` reaches it) raises an explicit certification error rather than
  a wrong value or an "unphysical" verdict. A realisation bug-fix only, not a change to the
  sealed §27.2 mathematics (`CONVENTION_VERSION` unchanged).
...
- **Dispatch GT3b — ion normal→local symplectic adapter (`ion_modes.py`).** New module building
  ...
  wire ordering, real orthonormal basis, `axes_per_ion ∈ {1,3}`, and the producer's
  `normalization_weighting_tags` — accepted and retained read-only) into a **genuinely immutable
  record (private read-only array copies)** — so the producer's `asdict()` feeds `materialize`
  directly; no shared runtime class with the producer
...
**Release summary.** A docs/accessibility minor bundling the tutorial-track
accessibility pass (WP-06) with the previously-merged WP-04 non-Markovianity
primitives and WP-05 non-adiabatic-squeezing Phase-A code and convention seal.
WP-06 adds Tutorial 0, a glossary, difficulty badges, a runnable Getting Started
example, a reusable pipeline diagram, and a newcomer rail (New-here boxes, symbols
tables, Common-confusion callouts, Takeaways, explanatory asserts) across
tutorials 00–18 (Tutorials 19–20 carry the difficulty badge here; their rail —
the original pilot — arrives with the forthcoming SQ6 merge). WP-04 adds
observable-only trace-distance, BLP non-Markovianity,
and QPN-bias primitives plus a Wittemer-2018 reproduction benchmark. WP-05 seals
`CONVENTIONS.md` §26 and ships the single-ion squeezing toolbox, Wigner/readout
façades, parity-aware truncation guard, benchmark, and Tutorials 19–20.

**Test surface at `v0.7.0`:** 21 tutorials execute under `pytest -m tutorial`,
notebook freshness is clean, `mkdocs build --strict` is clean, ruff/format/mypy
are clean. Two docs test modules guard the notebook-builder admonition transform
and the Getting Started runnable example.
...
- **Phase A / WI-1 (dispatch ND1) — trace distance + BLP non-Markovianity (observable-only).**
  ...
  Wittemer et al., PRA 97, 020102(R) (2018), Eq. 1). Re-exported through
  `information/` and the top-level package, following the `information/fisher.py`
  precedent. No new convention symbol and no frozen-section edit. Contract tests
  (`tests/unit/test_distinguishability.py`) cover metric properties, partial-trace
  reduction, and §15 failure paths; analytic oracle
...
- **Phase A / WI-3 (dispatch ND3) — QPN-bias estimator (observable-only).** New
  ...
  ℬ_sampling` with `𝒩_true = lim_{γ,r→∞} 𝒩` (Wittemer et al., PRA 97, 020102(R)
  (2018)). Re-exported through `information/` and the top-level package; no new
  convention symbol. Analytic oracle (`tests/regression/analytic/test_qpn_bias.py`)
  covers the positive QPN bias vanishing as `r→∞`, the negative coarse-`γ`
  sampling bias, and the telescoping decomposition.
- **Phase A / WI-4 (dispatch ND4) — single-mode Wittemer-2018 benchmark
  (observable-only).** New compute-only `tools/run_wittemer_2018_benchmark.py`
  reproduces the qualitative features of Wittemer et al., PRA 97, 020102(R)
  (2018) from the ND1/ND3 primitives on the single-mode
  `clos2016_spin_boson_hamiltonian`: trace-distance revivals, the `𝒩(δω_z)`
  local-probing resonance, the QPN-bias decomposition vs shots, and the
  suppression of `𝒩` by added (benchmark-local) spin decoherence. Writes a
  provenance-stamped artifact (with `calibration`/`generator` notes recording
  the `n̄`/Fock and `γ`-sampling regime) under `benchmarks/data/wittemer_2018/`;
  qualitative + ND3-consistency oracle in
  `tests/regression/analytic/test_wittemer_2018_benchmark.py`. No new convention
  symbol; spin `Γ_dec` injected as a benchmark-local `c_op` (WP-04 R5), normalized
  as `√(Γ_dec/2)·σ_z` so `Γ_dec` is the physical pure-dephasing rate (`= 1/T2`).
- Colab-executable notebooks for every tutorial (00–20), generated by
  `tools/build_tutorial_notebooks.py`, plus a `pytest -m tutorial` CI guard that
  executes each notebook so its inline asserts run.
- **WP-05 Phase A code — non-adiabatic squeezing toolbox (SQ1–SQ5, SQ7).**
  Single-ion time-dependent-frequency squeezing behind sealed `CONVENTIONS.md` §26:
  `hamiltonians.nonadiabatic_squeezing_hamiltonian`, `waveforms.FrequencyWaveform`
  with named shapes (`sinusoidal_modulation`, `gaussian_quench`, `smooth_ramp`),
  the N=1 covariance/symplectic core in `gaussian.py`, Wigner/readout façades in
  `phase_space.py`, `analytic.adiabaticity_parameter`, a parity-aware
  Fock-truncation guard, the Wittemer-2020 single-ion reproduction benchmark, and
  Tutorials 19–20. Dispatch family `SQ` minted; `CONVENTION_VERSION` 0.4 → 0.5.
- **WP-06 — tutorial-track accessibility (dispatches TA1–TA7).** Newcomer rail
  across tutorials 00–18 (Tutorials 19–20's rail — the original pilot — arrives
  with the forthcoming SQ6 merge): Tutorial 0 (30-second mental model), a
  cross-linked glossary (`docs/glossary.md`), `intro/core/advanced` difficulty
  badges and a "First time?" path on the tutorials index, a runnable first
  simulation in `docs/getting-started.md`, a reusable ASCII pipeline diagram,
  per-tutorial New-here boxes, symbols tables, Common-confusion callouts, figure
  Takeaways, boilerplate-vs-physics comments, and explanatory assert messages.
  `tools/build_tutorial_notebooks.py` extended to degrade `???`/`???+`
  collapsible admonitions to labelled blockquotes in Colab; guarded by
  `tests/docs/test_notebook_builder.py` and
  `tests/docs/test_getting_started_executes.py`.

### Changed

- **Conventions — WP-05 Phase A ratified and code shipped; `CONVENTION_VERSION`
  0.4 → 0.5; new §26 sealed.** `CONVENTIONS.md` gains §26 (non-adiabatic
  squeezing: time-dependent-frequency generator
  `H_sq/ℏ = −(i/4)(d ln ω/dt)(â†²−â²)` in a fixed `ω(0)` basis, vacuum-variance-1
  quadrature normalisation `x̂=â+â†`, Wigner `g`-pin).
  `WORKPLAN_v0.3.md` gains §5.8 dispatch-track stub; dispatch family `SQ` minted.
  Phase-A code (SQ1–SQ5, SQ7) landed on `main` via PR #1.

### Fixed

- `load_clos2016_*` raised `FileNotFoundError` on any installed copy (including
  Colab): the Clos 2016 reference tables resolved to a repo-only path and shipped
  in neither the wheel nor the sdist. They are now vendored as package data under
  `src/iontrap_dynamics/_data/clos2016_legacy/` and resolved via `__file__`, so the
  loaders work from any install (the `legacy_dir=` override still reads the archival
  bundle). Tutorial 13's "why we stop at N=3" scaling table was corrected to
  `d = 2·(n_c+1)^N`.
...
- **Dispatch EDB — `information`: quantum Darwinism (redundancy + recoverability).**
  ...
  helpers (entropy, ket→dm, index validation) used across `fisher` / `redundancy`
  / `recoverability`. Application-agnostic; conventions staged for CONVENTIONS §20
  under the shared v0.3 freeze. Verified in `tests/unit/test_redundancy.py`
  (GHZ-cascade Darwinism **plateau** `I(S:F) = H_S`, `R_δ = N`) and
  `tests/unit/test_recoverability.py` (perfect recovery → `H_S`, full
  decoherence → 0, monotone on a Werner dephasing family). Per
