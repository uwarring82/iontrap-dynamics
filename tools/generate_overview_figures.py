...
def _ion_trap_schematic(out_path: Path) -> None:
    ...
    ax.text(7.7, 1.0, "trap potential", ha="left", va="center",
            color=COLOR_TRAP, fontsize=10, style="italic")
    ...
    ax.text(ion_x + 0.32, ion_y, "²⁵Mg⁺", ha="left", va="center",
            color=COLOR_ION, fontsize=10)

    # Two laser beams approaching the ion.
    ax.annotate(
        "", xy=(ion_x - 0.18, ion_y + 0.15), xytext=(2.6, 3.4),
    ...
    ax.annotate(
        "", xy=(ion_x + 0.18, ion_y + 0.15), xytext=(7.4, 3.4),
        arrowprops=dict(arrowstyle="->", color=COLOR_LASER, linewidth=1.6),
    )
    ...
    ax.text(8.55, (g_y + e_y) / 2, "spin", ha="right", va="center",
            color=COLOR_SPIN, fontsize=10, style="italic")
    ...
    for y_level, lab in zip(mot_levels, mot_labels):
        ax.text(mot_x0 - 0.15, y_level, lab, ha="right", va="center", fontsize=10)
    ax.text((mot_x0 + mot_x1) / 2, 5.3, "motion", ha="center", va="bottom",
            color=COLOR_MOTION, fontsize=10, style="italic")

    # Title.
    ax.text(5.0, 5.7, "A single trapped ion: spin coupled to motion via lasers",
...
def _open_system_memory(out_path: Path) -> None:
    ...
    for i in range(n_grid):
        for j in range(n_grid):
            ax.add_patch(plt.Rectangle((j, n_grid - 1 - i), 1, 1,
                                       facecolor=COLOR_DENSITY, edgecolor="white",
    ...
    ax.text(n_grid / 2, n_grid + 0.5, r"density matrix $\rho$",
            ha="center", va="bottom", fontsize=12, fontweight="bold")
    ax.text(n_grid / 2, -0.5, r"memory $\propto$ dim$^{2}$",
            ha="center", va="top", fontsize=11)
    ax.text(n_grid / 2, -1.15,
    ...
    for k in range(n_traj):
        ...
        for i in range(n_states):
            ax.add_patch(plt.Rectangle((x0, n_states - 1 - i), 1, 1,
                                       facecolor=COLOR_TRAJ, edgecolor="white",
                                       linewidth=0.6, alpha=0.85))
        ax.text(x0 + 0.5, -0.3, f"#{k + 1}", ha="center", va="top",
    ...
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.text(n_traj * spacing / 2 - 0.4, n_states + 0.5,
            "trajectory ensemble (Monte Carlo)",
            ha="center", va="bottom", fontsize=12, fontweight="bold")
    ax.text(n_traj * spacing / 2 - 0.4, -0.7,
            r"memory $\propto$ dim per trajectory",
            ha="center", va="top", fontsize=11)
    ax.text(n_traj * spacing / 2 - 0.4, -1.25,
            "trajectories run independently in parallel\n— the workload shape GPUs are designed for",
            ha="center", va="top", fontsize=9.5, style="italic", color="#444444")

    # Top-level title across both panels.
    fig.suptitle("Two ways to simulate dissipative dynamics — and why the GPU answer differs",
